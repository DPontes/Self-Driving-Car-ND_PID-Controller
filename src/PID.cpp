...
void PID::Init(double Kp, double Ki, double Kd,
               double i_max, double d_max, double d_smooth, double error_rate_max) {
...
/*
  PID function to calculate total error output (P+I+D terms), with rate limit
  filtering and min/max guard to maintain smoother steering control.
...
double PID::TotalError() {
  ...
  double total_error_filt = RateLimit(total_error_raw, prev_total_error_, error_rate_max_);
...
/*
  Twiddle algorithm to decide next parameter set based on the twiddle error result.
  Twiddle steps are:
  ...
  if(twiddle_switch_ == false){   // test gain in the increased direction
    if(twiddle_error_ < twiddle_best_error_){
      ...
      *Kgains_[twiddle_idx_] += Kdeltas_[twiddle_idx_];
      ...
      std::cout << "Found new best error, boost delta and move to next index."
                << std::endl;
    }
    else{
      *Kgains_[twiddle_idx_] -= 2 * Kdeltas_[twiddle_idx_];
      twiddle_switch_ = true;   // Switch directions and try again
      std::cout << "Error is worse, try in the other direction." << std::endl;
    }
  }
  else{   // twiddle switch = true (retest gain in the decreased direction)
    if(twiddle_error_ < twiddle_best_error_){
      twiddle_best_error_     = twiddle_error_;
      Kdeltas_[twiddle_idx_] *= 1.2;
      // Found new best error, boost delta
      std::cout << "Found new best error, boost delta." << std::endl;
    }
    else{
      // Neither direction improved error, set gain back to original and reduce
      // size of delta
      *Kgains_[twiddle_idx_] += Kdeltas_[twiddle_idx_];
      Kdeltas_[twiddle_idx_] *= 0.0;
      std::cout << "Error still worse in this direction, set back gain and reduce delta."
                << std::endl;
    }
